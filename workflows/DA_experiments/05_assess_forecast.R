...
forecast_depth_skill <- data.frame(Depth=unique(forecast_depth$depth),RMSE=NA, pbias=NA, bias=NA, CRPS=NA, DA=names(date_list[da_freq]), day=daily[date])
...
readr::write_csv(forecast_depth_skill, file.path(paste0(lake_directory,"/forecasts/bvre/DA_experiments/",names(date_list)[da_freq],"/35d_depth_forecast_skill_",daily[date],".csv")))

#save daily rmse as csv
readr::write_csv(forecast_daily_avg_skill, file.path(paste0(lake_directory,"/forecasts/bvre/DA_experiments/",names(date_list)[da_freq],"/35d_daily_forecast_skill_",daily[date],".csv")))
