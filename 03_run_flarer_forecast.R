...
if(s3_mode){
  ...
  aws.s3::save_object(object = file.path(forecast_site, config$met$hist_met_file), bucket = "targets", file = file.path(config$file_path$qaqc_data_directory, config$met$hist_met_file))
...
observed_met_file <- file.path(config$file_path$qaqc_data_directory, config$met$hist_met_file)
