...
combine_bvre_insitu <- function(lake_directory, config_obs){
  bvrheader1 <- read.csv(file.path(lake_directory, "data_raw", config_obs$realtime_insitu_location, "BVRplatform.csv"), skip = 1, as.is = T) #get header minus wonky Campbell rows
  bvrdata1 <- read.csv(file.path(lake_directory, "data_raw", config_obs$realtime_insitu_location, "BVRplatform.csv"), skip = 4, header = F) #get data minus wonky Campbell rows
  ...
  #current BVR data
  bvrheader5<-read.csv(file.path(lake_directory, "data_raw", config_obs$realtime_insitu_location, "bvre-waterquality.csv"), skip=1, as.is=T) #get header minus wonky Campbell rows
  bvrdata5<-read.csv(file.path(lake_directory, "data_raw", config_obs$realtime_insitu_location, "bvre-waterquality.csv"), skip=4, header=F) #get data minus wonky Campbell rows
  ...
  #write as a new file 
  write.csv(bvrdata_new, file.path(lake_directory, "data_raw","bvrewaterquality.csv"), row.names = F)
}
